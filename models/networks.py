...
def define_G(input_nc, output_nc, ngf, netG, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netG == 'resnet_9blocks':
        ...
        net = RobustGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6)
    else:
...
class ContentLoss(nn.Module):
    ...
    def __call__(self,  out_images, target_images):
        # Perception Loss
        perception_loss = self.mse_loss(self.loss_network(out_images), self.loss_network(target_images))
        return 0.005 * perception_loss
...
class NLayerDiscriminator(nn.Module):
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.utils.spectral_norm):
        ...
        for n in range(1, n_layers):  # gradually increase the number of filters
            ...
            if n==1:
                sequence += [SelfAttention(128, 'relu')] 
